...
%             my( $type, $class) = resolve_schema( $content->{$ct}->{schema}, $prefix,$schema,$schema_file );
%             if( $type eq 'string' ) {
...
% my( $vtype, $class) = resolve_schema( $type, $prefix,$schema,$schema_file );
...
[ map { <%= include('inflated_response', { type => $type->{items}, prefix => $prefix, argname => '$_', schema => $schema, schema_file => $schema_file }) %> } $payload->@* ],
% } else {
<%= $argname %>
% }
...
% my $type = $info->{content}->{$ct}->{schema};
                            <%= include('inflated_response', { type => $type, prefix => $prefix, argname => '$payload', schema => $schema, schema_file => $schema_file } ) %>
                        );
                    };
...
% my @included_types;
% my ($type, $info) = resolve_schema($elt, $prefix,$schema,$schema_file);
% my @subclasses = openapi_dependencies( type => $elt, prefix => $prefix, schema => $schema, schema_file => $schema_file );
%
package <%= $prefix %>::<%= $name %> <%= $version %>;
# DO NOT EDIT! This is an autogenerated file.
use 5.020;
...
%                    my $descriptor = 'a';
%                    my $class;
%                    my ($type,$class_info) = resolve_schema( $schema, $prefix,$schema,$schema_file );
%                    if( $type eq 'array' ) {
...
<%= include( 'required_parameters', { elt => $elt, schema => $schema, schema_file => $schema_file }); =%>
%#
...
%=include('return_types', { prefix => $prefix, elt => $elt, schema => $schema, schema_file => $schema_file });
=cut

<%= include( 'build_request', { method => $method, parameters => \%parameters, elt => $elt, ct => $ct, prefix => $prefix, schema => $schema, schema_file => $schema_file} ); %>
